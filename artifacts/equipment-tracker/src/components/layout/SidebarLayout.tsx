...
import { Map, LayoutDashboard, Server, Settings, ScanLine } from "lucide-react";
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden selection:bg-primary/20">
      ...
      <div className="flex-1 flex flex-col h-full min-w-0 overflow-hidden">
        ...
        {/* Mobile bottom nav */}
        <nav className="md:hidden flex border-t bg-card flex-shrink-0" data-testid="mobile-bottom-nav">
          {navigation.map((item) => {
            const isActive = item.href === "/" ? location === "/" : location.startsWith(item.href);
            return (
              <Link
